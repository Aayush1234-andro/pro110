...
while (cap.isOpened()):
    ...
    if results.multi_hand_landmarks: # Getting Landmark(location) of Hands if Exists 
        ...
        if lmList[thumbCoordinate[0]][0] > lmList[thumbCoordinate[1]][0]:
            upcount+=1
        if upcount == 2:
            cv2.putText(img, "Scissors", (20, 20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255))
        elif upcount == 0:
            cv2.putText(img, "Rock", (20, 20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255))
        elif upcount == 5:
            cv2.putText(img, "Paper", (20, 20), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255))    
